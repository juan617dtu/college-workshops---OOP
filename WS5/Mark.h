...
namespace seneca {
   ...
   public:
      ...
      friend double operator/(double value, Mark& mark);
      friend int operator/(int value, Mark& mark);
   };

   // student helper function prototypes go here
   std::ostream& display(const Mark& mark, char display_type, std::ostream& os = std::cout);
